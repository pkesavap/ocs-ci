...
def add_million_files(pod_name):
    """
    ...
    ocp_obj.exec_oc_cmd(
        f'rsync {ntar_loc} {pod_name}:{constants.MOUNT_POINT}',
        timeout=300
    )
    ocp_obj.exec_oc_cmd(
        f'exec {pod_name} -- mkdir {constants.MOUNT_POINT}/x'
    )
    ocp_obj.exec_oc_cmd(
...
class MillionFilesOnCephfs(object):
    ...
    def __init__(self):
        ...
        self.test_file_list = add_million_files(self.pod_name)
...
def million_file_cephfs(request):
    million_file_cephfs_obj = MillionFilesOnCephfs()
    retry(
    ...
    def teardown():
        million_file_cephfs_obj.cleanup()
        retry(
            exceptions.CephHealthException,
            tries=60, delay=5, backoff=1
        )(ceph_health_check)()
    ...
    return million_file_cephfs_obj
